...
import org.finos.vuu.core.table.{Column, DataType, RowData, RowWithData}

trait InMemColumnIndices {

  def indexForColumn(column: Column): Option[IndexedField[_]]

  def insert(rowWithData: RowWithData): Unit

  def update(originalRow: RowWithData, updatedRow: RowWithData): Unit

  def remove(rowWithData: RowWithData): Unit
...
private case class InMemColumnIndicesImpl(indices: Map[Column, IndexedField[?]], 
  ...
  override def remove(rowWithData: RowWithData): Unit = {
    val rowKey = rowWithData.key

    var i = 0
    while (i < updaters.length) {
      val updater = updaters(i)
      val value = rowWithData.get(updater.column)
      if (value != null) {
  ...
  override def update(originalRow: RowWithData, updatedRow: RowWithData): Unit = {
